...
interface AppState {
  ...
  toggleLocale: () => void;
  toggleDarkMode: () => void;
  login: (loginId: string, password: string, role?: 'buyer' | 'seller' | 'admin') => Promise<void>;
  register: (name: string, email: string, phone: string, password: string, role: 'buyer' | 'seller' | 'admin') => Promise<void>;
  logout: () => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  login: async (loginId, password, role) => {
    ...
    // Set up user object from token/data
    const me = await fetchApi('/users/me');

    // Make sure we reject non-admin logins for the admin portal if we want
    if (me.role !== 'admin') {
  ...
  register: async (name, email, phone, password, role) => {
    await fetchApi('/auth/register', {
      ...
      body: JSON.stringify({
        ...
        password,
        full_name: name,
        role: 'admin', // Force admin role for this portal
      }),
    });
    
    // Automatically log in after register
    await get().login(email || phone, password, 'admin');
  },
